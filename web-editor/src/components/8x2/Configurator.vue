<template>
  <div class="configurator">
    <div ref="interfaceSelectorContainer" :class="['interface-selector-container', { scrolling: scrolling }]">
      <div class="interface-selector">
        <div class="relative-position interface-components">
          <TabsSelector v-model="midiInterface" :options="['usb', 'trs'].map((value) => ({ label: value, value }))"
            label="Interface" padding="4ch" shrink :class="['selector', { scrolling: scrolling }]" />
    ...
    <div :class="['configurator-body', { scrolling: scrolling }]">
      <div class="row">
        <TabsSelector v-if="midiBank != null" ref="bankSelector" v-model="midiBank"
          :options="Banks.map((value) => ({ label: value.toString(), value }))" label="Bank" vertical
        ...
        <div class="pots-config relative-position">
          <MappingsGroup v-model:ccs="firstEightCcs" v-model:channels="firstEightChannels"
            v-model:channelLink="linkChannels" :on-device-ccs="onDeviceFirstEightCcs"
            :on-device-channels="onDeviceFirstEightChannels" :potentiometers="firstEightPotentiometers"
            @granular-update="processGranularUpdate($event)" />

          <div style="width: 100%; margin-top: 2rem" />

          <MappingsGroup v-model:ccs="lastEightCcs" v-model:channels="lastEightChannels"
            v-model:channelLink="linkChannels" :on-device-ccs="onDeviceLastEightCcs"
            :on-device-channels="onDeviceLastEightChannels" :potentiometers="lastEightPotentiometers" :count-offset="8"
            @granular-update="processGranularUpdate($event)" />
        </div>
      </div>
...
import ConfigUploadButton from 'components/8x2/ConfigUploadButton.vue';
import HachiNi from 'components/8x2/HachiNi.vue';
import type { GranularUpdate } from 'components/8x2/MappingsGroup.vue';
import MappingsGroup from 'components/8x2/MappingsGroup.vue';
import SettingsMenu from 'components/8x2/SettingsMenu.vue';
import TabsSelector from 'components/8x2/TabsSelector.vue';
import { Banks } from 'access/types';
import type { Bank, DeviceInfo, Interface } from 'access/types';
import { useModal } from 'composables';
import CopyInterfaceConfig from 'components/8x2/CopyInterfaceConfig.vue';
...
onMounted(() => {
  observer.value = new IntersectionObserver(([e]) => (scrolling.value = (e?.intersectionRatio ?? 1) < 1), {
    threshold: 0.999,
    root: null,
  });
